...
A **Plan usage** row sits under the subscription accounts, showing the active account's 5-hour session limit — the one that actually stops work — as a percentage and its reset time. Its icon is a three-bar chart that lights up as you spend: one green bar, two orange, three red, so the row reads at a glance even without telling the colours apart. A 5-hour window has no reset time until it starts, so an unstarted one reads *not started* rather than implying a deadline. Everything worth knowing is on the row itself — there is no hover text to discover.

If a refresh fails while the last good numbers are still on screen, the row shows a small orange warning triangle, so an unchanged percentage is never mistaken for a broken Refresh; the reason — a rate limit, a rejected login, or the API having trouble — is in the submenu. Only the account you're actually on is polled, since the others aren't spending anything, so they show whatever was last read for them; `claude-billing usage` on its own still checks every account.

The submenu breaks down every registered account: a bar and percentage per limit, usage credits spent against your monthly cap when enabled, your prepaid credit balance with the soonest expiry, and, beside each account's name, when its figures were last updated. Each limit's bar fills toward it — green below 60%, orange from 60%, red from 85% — so how close you are reads before the numbers do.
