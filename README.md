...
A **Plan usage** row sits under the subscription accounts, showing the active account's 5-hour session limit — the one that actually stops work — as a percentage followed by a bar that fills toward the limit, with the limit's name and reset time on hover. Its submenu breaks down every registered account: a bar and percentage per limit, extra usage credits when enabled, and how stale the figures are. Bars are green below 60%, orange from 60%, and red from 85%, so how close you are reads before the numbers do. It refreshes on its own slower timer so a network call never delays the billing state, and the menu's **Refresh** action forces a usage fetch too. The numbers come from an unofficial endpoint — see [Subscription plan usage](#subscription-plan-usage) for the caveats.
