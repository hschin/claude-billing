...
A **Plan usage** row sits under the subscription accounts, showing the active account's 5-hour session limit — the one that actually stops work — as a percentage and its reset time, followed by a bar that fills toward the limit; the limit's name is on hover. A 5-hour window has no reset time until it starts, so an idle one reads *no active window* rather than implying a deadline. Its submenu breaks down every registered account: a bar and percentage per limit, usage credits spent against your monthly cap when enabled, and how stale the figures are. Bars are green below 60%, orange from 60%, and red from 85%, so how close you are reads before the numbers do. It refreshes on its own slower timer so a network call never delays the billing state, and the menu's **Refresh** action forces a usage fetch too. The numbers come from an unofficial endpoint — see [Subscription plan usage](#subscription-plan-usage) for the caveats.
