...
A **Plan usage** row sits under the subscription accounts, showing the active account's 5-hour session limit — the one that actually stops work — as a percentage and its reset time; the limit's name is on hover. Its icon is a three-bar chart that lights up as you spend: one green bar, two orange, three red, so the row reads at a glance even without telling the colours apart. A 5-hour window has no reset time until it starts, so an unstarted one reads *not started* rather than implying a deadline. If a refresh fails while the last good numbers are still on screen, the row shows an orange warning triangle, so an unchanged percentage is never mistaken for a broken Refresh; hover it, or open the submenu, for the reason — a rate limit, a rejected login, or the API having trouble. Only the account you're actually on is polled, since the others aren't spending anything, so they show whatever was last read for them; `claude-billing usage` on its own still checks every account. When an account's access token has expired, its bars go grey and the percentages blank out — the last figures read are no longer current, and a stale percentage shown as a live one is worse than none — with a note under them asking you to switch to that account once so Claude Code can refresh the token. The credit balance stays, since money in the account doesn't go stale the way a rolling window does. Its submenu breaks down every registered account: a bar and percentage per limit, usage credits spent against your monthly cap when enabled, your prepaid credit balance with the soonest expiry, and, beside each account's name, when its figures were last updated. In the submenu, each limit gets a bar that fills toward it, green below 60%, orange from 60%, and red from 85%, so how close you are reads before the numbers do. The row also says how long ago the figures were read. Everything in the menu refreshes on the same five-minute beat, and opening the menu refreshes the billing state immediately, so what you see when you look is current; **Refresh** forces everything, usage included. The numbers come from an unofficial endpoint — see [Subscription plan usage](#subscription-plan-usage) for the caveats.
...
# Subscription plan usage:
#   work:
#     5-hour session: 72% — resets 2026-08-14 16:00:00 UTC
#     Weekly (all models): 41% — resets 2026-08-19 01:59:59 UTC
#     Credit balance: $88.71 — $82.79 expires 2026-09-19
#     Usage credits: 86% used — $17.20 of $20.00
#   personal: access token expired — switch to this account to refresh it
```

The prepaid credit balance comes from a second request, to `api.anthropic.com/api/oauth/organizations/<org>/prepaid/credits`, which needs the account's organization UUID — taken from `~/.claude.json` for the active account and from its stashed identity for the others. Accounts without credits, or without a known organization, simply omit the balance; a refusal there never costs you the plan figures.

**This uses unofficial endpoints.** Claude Code's `/usage` view is the authoritative source. claude-billing calls the same `api.anthropic.com/api/oauth/usage` request that view makes, with your subscription OAuth token, plus the credits endpoint above. Neither is a documented, versioned API: they can change or disappear at any time. When it does, usage reports as unavailable — it never blocks a billing switch and never invents a number. Only the normalized `limits` array and `spend` object are read; the other keys in the response are internal.
