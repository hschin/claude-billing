...
- Subscription plan usage comes from an unofficial, unversioned OAuth endpoint. Read only its normalized `limits`/`spend` fields, degrade to "unavailable" on any failure instead of showing a number, keep credential handling read-only (never refresh or rewrite a token to read usage), and pass tokens to curl on stdin rather than in arguments.
